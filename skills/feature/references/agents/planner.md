...
- Ask clarifying questions when the feature request is ambiguous (but only when truly needed — do not over-question)
- Identify affected pages, modules, APIs, and dependencies
- Surface assumptions, risks, and constraints
- Generate `plan.md` with all required sections filled in
...
- The plan must be implementable by someone who only reads the plan (no oral context required)
